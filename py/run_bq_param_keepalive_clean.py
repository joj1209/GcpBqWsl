#!/usr/bin/env python3
"""run_bq_param_keepalive_clean.py

Purpose:
- Same behavior as py/run_bq_param.py, but shows a periodic keepalive while a
  long-running `bq query` is executing.
- Default output refreshes a *single console line* (no growing lines).

Env vars:
- BQ_HEARTBEAT_SEC: seconds between heartbeat updates (default: 2)
- BQ_QUIET: if true, add `--quiet` to bq query (default: 1 to preserve old behavior)
- BQ_HEARTBEAT_STYLE: `inline` (default) refreshes one console line, `log` prints new log lines
- BQ_HEARTBEAT_ANSI: if true, use ANSI clear-line for inline refresh (default: auto via TTY)

Examples:
- Default (quiet + inline heartbeat every 2s):
  python3 py/run_bq_param_keepalive_clean.py mid=... vs_job_dt=20260211

- Let bq print its own status updates too (may interleave output):
  BQ_QUIET=0 python3 py/run_bq_param_keepalive_clean.py ...

- Heartbeat as log lines (will grow lines):
  BQ_HEARTBEAT_STYLE=log python3 py/run_bq_param_keepalive_clean.py ...
"""
...
def setup_logging(base_dir: Path) -> Tuple[Path, Path]:
    ...
    base = "run_bq_param_keepalive_clean.{}.{}".format(stamp, os.getpid())
    ...
    # Send logs to stderr so the inline heartbeat on stdout stays clean.
...
# ============================
def _clear_inline(last_len: int, ansi: bool) -> None:
    if ansi:
...
def run_bq_query_with_params(
    ...
    heartbeat_ansi = _env_bool("BQ_HEARTBEAT_ANSI", sys.stdout.isatty())

    heartbeat_style = os.getenv("BQ_HEARTBEAT_STYLE", "inline").strip().lower()
    if heartbeat_style not in ("inline", "log"):
        heartbeat_style = "inline"
    ...
    try:
        while True:
            try:
                ...
                if heartbeat_style == "log":
                    logger.info("bq query running... elapsed=%02d:%02d:%02d", hh, mm, ss)
                    continue

                msg = f"{spinner[spin_i % len(spinner)]} running {hh:02d}:{mm:02d}:{ss:02d}"
                spin_i += 1
                if heartbeat_ansi:
        ...
        if heartbeat_style == "inline":
            _clear_inline(last_len, heartbeat_ansi)
