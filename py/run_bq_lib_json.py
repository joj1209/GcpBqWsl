...
def run_bq_query(sql_text: str) -> None:
    """Execute BigQuery query using BigQuery Python client."""
    if bigquery is None:
        raise RuntimeError(
            "Missing dependency: google-cloud-bigquery. "
            "Install with: pip install google-cloud-bigquery"
        )
    if not hasattr(run_bq_query, "_client"):
        run_bq_query._client = bigquery.Client()

    job = run_bq_query._client.query(sql_text)
...
def execute_sql_jobs(targets: List[Dict[str, str]], overrides: Dict[str, str]) -> Tuple[int, int, int]:
    ...
    for record in targets:
        ...
        try:
            ...
            logger.error("bq query failed (%s): %s", type(e).__name__, e)
            fail += 1
